 ...
 */
public class RibbonJGroupsArchiveTest {

    @Test
    public void testAdvertiseDefaultName() {
        JARArchive archive = ShrinkWrap.create(JARArchive.class, "myapp.war");
        archive.as(RibbonJGroupsArchive.class)
                .advertise();

        Asset asset = archive.get(RibbonJGroupsArchive.RIBBON_APP_CONF_PATH).getAsset();

        assertThat(asset).isNotNull();
        assertThat(asset).isInstanceOf(StringAsset.class);
        assertThat(((StringAsset) asset).getSource().trim()).isEqualTo("myapp");

        assertThat( archive.as(ServiceActivatorArchive.class).containsServiceActivator( RibbonJGroupsArchiveImpl.SERVICE_ACTIVATOR_CLASS_NAME )).isTrue();
    }
    ...
    public void testNotAdvertise() {
        ...
        assertThat( archive.as(ServiceActivatorArchive.class).containsServiceActivator( RibbonJGroupsArchiveImpl.SERVICE_ACTIVATOR_CLASS_NAME )).isFalse();
    }
